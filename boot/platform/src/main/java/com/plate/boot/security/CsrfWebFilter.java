...
public class CsrfWebFilter implements WebFilter {
     ...
     */
    @Override
    public @NonNull Mono<@NonNull Void> filter(ServerWebExchange exchange, @NonNull WebFilterChain chain) {
        log.debug("{}Csrf filter chain [CsrfWebFilter] next.", exchange.getLogPrefix());
        Mono<@NonNull CsrfToken> csrfTokenMono = exchange.getAttribute(CsrfToken.class.getName());
