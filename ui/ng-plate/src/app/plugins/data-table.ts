        ...
        <div class="ms-auto">
          <div class="input-icon">
            ...
            <span class="input-icon-addon">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  @if (column.sortable) {
                    <span class="ms-1">
                      @if (sortColumn() === column.key && sortDirection() === 'asc') {
                        <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-chevron-up" width="16" height="16" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                          <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
      ...
      @if (showPagination()) {
        <div class="card-footer d-flex align-items-center">
          <div class="text-muted">
            显示第 {{ (currentPageIndex() * pageSize()) + 1 }} - 
            {{ Math.min((currentPageIndex() + 1) * pageSize(), filteredData().length) }} 条，
            共 {{ filteredData().length }} 条
          </div>
          <div class="ms-auto">
            <ul class="pagination mb-0">
              <li class="page-item" [class.disabled]="currentPageIndex() === 0">
                <a class="page-link" href="javascript:void(0)" (click)="goToPage(currentPageIndex() - 1)" aria-label="Previous">
                  <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
              ...
              @for (page of pages(); track $index) {
                <li class="page-item" [class.active]="page === currentPageIndex()">
                  <a class="page-link" href="javascript:void(0)" (click)="goToPage(page)">{{ page + 1 }}</a>
                </li>
              }
              
              <li class="page-item" [class.disabled]="currentPageIndex() === totalPages() - 1">
                <a class="page-link" href="javascript:void(0)" (click)="goToPage(currentPageIndex() + 1)" aria-label="Next">
...
export class DataTableComponent {
  ...
  // Internal state
 private filterTextValue = signal<string>('');
  private sortColumnValue = signal<string | null>(null);
  private sortDirectionValue = signal<'asc' | 'desc'>('asc');
  private currentPageIndexValue = signal<number>(0);
  
  // Computed properties
  filterText = computed(() => this.filterTextValue());
  sortColumn = computed(() => this.sortColumnValue());
  sortDirection = computed(() => this.sortDirectionValue());
  currentPageIndex = computed(() => this.currentPageIndexValue());
  ...
  pages = computed(() => {
    ...
    // Show max 5 pages in pagination, centered around current page
  ...
  onFilterChange(event: any) {
    this.filterTextValue.set(event.target.value);
    this.currentPageIndexValue.set(0); // Reset to first page when filtering
  }
  
  sort(columnKey: string) {
    if (this.sortColumn() === columnKey) {
      // If clicking the same column, toggle direction
      this.sortDirectionValue.set(this.sortDirection() === 'asc' ? 'desc' : 'asc');
    } else {
    ...
    this.currentPageIndexValue.set(0); // Reset to first page when sorting
  }
  ...
  // Expose Math object for use in template
  Math = Math;
}